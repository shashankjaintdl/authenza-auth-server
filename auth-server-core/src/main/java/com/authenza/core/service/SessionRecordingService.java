...
public class SessionRecordingService {
    ...
    public String recordSession(Long userId, HttpServletRequest request) {
        ...
        // Determine session TTL: use tenant override if exists, otherwise platform
        // default
        ...
        try {
            ...
            log.info("[Session] Recorded session for user ID {} from '{}' ({}) in tenant '{}'",
                    userId, ipAddress, deviceName, TenantContextHolder.getTenantId());

        } catch (Exception e) {
            // Non-fatal — session tracking must never block login
            log.error("[Session] Failed to record session for user ID {}: {}", userId, e.getMessage());
        }
