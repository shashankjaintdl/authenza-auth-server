 ...
 * Strategy:
 * <ol>
 * <li><strong>Attempt counter</strong> — stored in Redis with a TTL equal to
 * the lockout
 * duration. Key pattern: {@code bf:{tenantId}:{username}}</li>
 * <li><strong>Account lock</strong> — when the counter reaches
 * {@code maxAttempts}, the
 * user's row in {@code application_user} is updated:
 * {@code status = 'LOCKED'} and
 * {@code locked_until = NOW() + lockDuration}.</li>
 * <li><strong>Auto-unlock</strong> — if {@code locked_until} is in the past
 * when
 * {@link JdbcTenantUserDetailsService} loads the user, it calls
 * {@link #clearLock} to transparently restore the account.</li>
 * <li><strong>Reset on success</strong> — a successful authentication deletes
 * the Redis
 * key and resets the DB counters.</li>
 * </ol>
 *
 * <p>
 * The tenant-scoped Redis key prevents bleed between tenants sharing the same
 * Redis
 * instance.
 */
@Service
public class BruteForceProtectionService {
     ...
     *
     * <p>
     * When the attempt count reaches {@code maxAttempts}, the account is locked
    ...
    public boolean recordFailedAttempt(String username, String tenantId) {
        String isEnabledStr = tenantSettingsCache.getSetting(tenantId, "brute_force_protection_enabled");
        boolean isEnabled = isEnabledStr == null || Boolean.parseBoolean(isEnabledStr); // Default true

        if (!isEnabled) {
            log.info("[BruteForce] Protection is disabled for tenant '{}', ignoring failed attempt", tenantId);
            return false;
        }

        String maxAttemptsStr = tenantSettingsCache.getSetting(tenantId, "brute_force_max_attempts");
        int maxAttempts = properties.getMaxAttempts(); // fallback
        if (maxAttemptsStr != null) {
            try {
                maxAttempts = Integer.parseInt(maxAttemptsStr);
            } catch (NumberFormatException e) {
                log.warn("[BruteForce] Invalid max_attempts '{}' for tenant '{}', using fallback", maxAttemptsStr,
                        tenantId);
            }
        }
        ...
        if (attempts != null && attempts >= maxAttempts) {
            lockAccountInDb(username, tenantId, duration, maxAttempts);
     ...
     * Also clears any stale lock in the DB (in case they were unlocked by TTL
     * expiry).
     *
     ...
     *
     * <p>
     * Called automatically by {@link JdbcTenantUserDetailsService} when
    ...
    public void clearLock(String username, String tenantId) {
        ...
        if (updated > 0) {
            // If the account was successfully unlocked, ensure the Redis counter is also
            // wiped completely clean
            String key = buildKey(tenantId, username);
            redisTemplate.delete(key);
            log.info("[BruteForce] Account UNLOCKED and counters reset — tenant={} username={}", tenantId, username);
        }
    }
    ...
    /**
     * Persists the lock into the DB for the given username within the current
     * tenant context.
     */
    private void lockAccountInDb(String username, String tenantId, long durationMinutes, int maxAttempts) {
